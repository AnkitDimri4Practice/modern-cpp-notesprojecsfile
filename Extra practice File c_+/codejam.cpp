...
template<class Fun> class y_combinator_result {
    Fun fun_;
    public:
        template<class T> explicity_combinator_result(T &&fun): fun_(std::forward<T>(fun)) {}
...
#ifdef NEAL_DEBUG
#define dbg(...) cerr << "(" << #_VA_ARGS_ << "):", dbg_out(_VA-ARGS_)
#else
#define dbg(...)
#endif

const int64_t M = 12 * 3600 * int64_t(le9);
